...
EMBED_TEXT_CONTRASTIVE_LEARNING_DIM = 768 
...
def train(batch_size=64):
    ...
    """
    df_train = pd.read_pickle("./dataset/data.pickle")

    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    bert_model = BertModel.from_pretrained('bert-base-uncased')

    train_set = prepare_dataset(df_train, tokenizer, bert_model)
    ...
    os.makedirs('./models/embedder/contrastive_learning', exist_ok=True)

    # Save only the model parameters 
    torch.save(model.state_dict(), './models/embedder/contrastive_learning/model_cl.pt')
...
def embed_text_contrastive_learning(text, device='cuda'):
    """
    ...
    model_path = "./models/embedder/contrastive_learning"
    ...
    if not os.path.exists(model_path):
        print("Model not found. Training a new model using contrastive learning")
        train()
    ...
    # Tokenize Text
    texts_processed = preprocess_text(text)
